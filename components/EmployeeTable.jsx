...
const EmployeeTable = (props) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      setData(Object.keys(data).reduce((result, key) => {
        if (key !== selectedId) {
          result[key] = data[key];
        }
        return result;
      ...
      <table className="w-full border text-sm text-center">
        <thead className="text-xs text-gray-700 uppercase bg-gray-100 sticky top-0">
          <tr>
            ...
            {/* <th scope="col" className="px-3 py-6 w-20">Part ID</th> */}
          ...
          {Object.keys(data).map((key) => (
            <React.Fragment key={key}>
              <tr onClick={() => handleKeyClick(key)} className="bg-white border-b cursor-pointer hover:bg-gray-200">
                ...
                {/* <td className="py-2 h-24">{data[key][0].part_id}</td> */}
                ...
                (data[key].length > 1 ? (
                  data[key].slice(1).map((datas, index) => (
                    <tr key={index} className={`bg-white border-b hover:bg-gray-200 ${selectedKey === key ? 'expand-row' : ''}`}>
                      <td></td>
                      {/* <td className="py-2 h-24">{datas.part_id}</td> */}
