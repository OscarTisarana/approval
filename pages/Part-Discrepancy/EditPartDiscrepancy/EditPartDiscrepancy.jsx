...
const EditPartDiscrepancy = () => {
    ...
    const getDataTable = async () => {
      ...
      await axios.get(`${PART_SERVICE_BASE_URL}part_discrepancy?user_id=${user.UserId}&search_id=${location.state.key}`) // belum kelar
          .then((res) => {
    ...
    const EditPartDiscrepancyAction = (e) => {
        //setValidationErrors({})
        e.preventDefault();
        setIsLoading(true);
        if(dataTable.length === 0){
          ...
          let payload = {
              PartDiscrepancyId: parseInt(location.state.key),
          ...
          axios.put(`${PART_SERVICE_BASE_URL}part_discrepancy/update`, payload)
          .then((r) => {
              console.log(r)
              if (r.status === 200) {
                setShowSuccess(true);
    ...
    return (
      ...
      <div className="container md:pl-40 flex lg:flex-row flex-col pt-32 justify-center">  
        <div className="lg:basis-1/3 pl-3">
          <form name="edit-part-Discrepancy" onSubmit={(e) => {EditPartDiscrepancyAction(e)}}>           
              <h1 className="text-3xl font-bold mb-4 text-start">Edit Part Discrepancy</h1>
...
export default EditPartDiscrepancy;